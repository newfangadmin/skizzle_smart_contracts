...
contract Skizzle is Initializable {
    ...
    * @return bool
    */
    function createDID(bytes32 _id, address _identity, bytes32 _file, uint256 n, uint256 k, uint256 file_size) onlyNode internal returns (bool){
        ...
        files[_file] = File(n, k, file_size, msg.sender, "");
        nonce[_identity]++;
        return true;
    }
    ...
    * @return bool
    */
    function createDID(bytes32 _id, bytes32 _file, uint256 n, uint256 k, uint256 file_size) public returns (bool){
        return createDID(_id, msg.sender, _file, n, k, file_size);
    }

    function createDIDSigned(bytes32 _id, bytes32 _file, uint256 n, uint256 k, uint256 file_size, address signer, uint8 v, bytes32 r, bytes32 s) public returns (bool) {
        bytes32 payloadHash = keccak256(abi.encode(_id, nonce[signer]));
        address actualSigner = getSigner(payloadHash, signer, v, r, s);
        return createDID(_id, actualSigner, _file, n, k, file_size);
    }
