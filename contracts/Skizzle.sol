...
contract Skizzle is Initializable {
    ...
    event create(address identity, bytes32 file, bytes32 doc, uint256 size);
    ...
    function createSigned(
        ...
        bytes32 payloadHash = keccak256(abi.encode(_file, _doc, _size, _fph, _nonce));
        getSigner(payloadHash, signer, v, r, s);
        // Just to check signature is valid or not.
        docs[_file] = File(signer, _doc, _size, _fph);
    ...
    function updateSigned(
        ...
        // Just to check signature is valid or not.
        bytes32 payloadHash = keccak256(abi.encode(_files, _docs, _nonce));
        getSigner(payloadHash, signer, v, r, s);
        for (uint i = 0; i < _files.length; i++) {
